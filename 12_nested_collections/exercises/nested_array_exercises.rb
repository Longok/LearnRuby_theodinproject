...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.each do |hash|
    hash.each do |key,value|
      if key == :is_my_favorite? && value == true
       return hash    #return: because no container or method being initiated in order to store or return those results.
      end
    end
  end
  return nil
  ...
  #cach 2:
  output = nil
  array_of_hash_objects.each {|hash| hash[:is_my_favorite?] ? output = hash : output }
  output
end
